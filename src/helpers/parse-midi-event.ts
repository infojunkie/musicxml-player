...
const _parseMetaEvent = (dataView: DataView, offset: number): { event: TMidiMetaEvent; offset: number } => {
    ...
    if (metaTypeByte === 0x01) {
        ...
        throw new Error(`Cannot parse a meta event with a type of "${_hexifyNumber(metaTypeByte)}"`);
    }
...
const _parseMidiEvent = (
    ...
    if (eventType === 0x08) {
        ...
        throw new Error(`Cannot parse a midi event with a type of "${_hexifyNumber(eventType)}"`);
    }
 ...
 */
const _hexify = (dataView: DataView, offset = 0, length = dataView.byteLength - (offset - dataView.byteOffset)) => {
 ...
 */
const _stringify = (dataView: DataView, offset = 0, length = dataView.byteLength - (offset - dataView.byteOffset)) => {
    const byteOffset = offset + dataView.byteOffset;
